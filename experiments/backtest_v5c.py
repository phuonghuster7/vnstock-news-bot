"""
Phase 5c: Backtest Nâng cao - Bước 2
- Sử dụng Signal từ V4 (Pure momentum).
- KHÔNG dùng Market Regime Filter.
- Cắt lỗ động (ATR Stop Loss): Cắt khi giá rớt xuống Entry - 2.0 * ATR(14).
- TOPK = 5.
"""
import os
import pandas as pd
...
# ─── 1. Stop-Loss Handler (ATR) ──────────────────────────────────────────────
class StopLossHandler:
    """
    Position-level stop loss using ATR.
    Stop = entry_price - 2.0 * ATR(14)
    """
    def __init__(self, atr_multiplier: float = 2.0, atr_window: int = 14):
        self.atr_mult = atr_multiplier
        self.atr_window = atr_window
        self.positions = {}  # symbol -> {"entry": price, "stop": price}
        
        # Load High, Low, Close for ATR calculation
        self.hlc_df = D.features(D.instruments("vn30"), ["$high", "$low", "$close"], start_time="2023-11-01", end_time=TEST_END)
        self.hlc_df.columns = ["high", "low", "close"]

    def _calc_atr(self, symbol: str, current_date: pd.Timestamp) -> float:
        try:
            sym_df = self.hlc_df.loc[symbol].loc[:current_date]
            if len(sym_df) < self.atr_window + 1:
                return 0.0
    ...
    def add_position(self, symbol: str, entry_price: float, current_date: pd.Timestamp):
        atr = self._calc_atr(symbol, current_date)
        if np.isnan(atr) or atr == 0:
            stop = entry_price * 0.93  # Fallback 7% if ATR fails
        else:
...
def run_backtest_v5c(pred_df: pd.DataFrame, price_map: dict, test_dates: list):
    holdings     = {}     # {sym: shares}
    ...
    stop_handler = StopLossHandler(atr_multiplier=2.0)
    ...
    for i, date in enumerate(test_dates):
        ...
        for sym in holdings.keys():
            current_prices[sym] = get_price(date, sym)
            
        # 2. Check Stop Loss FIRST
        stop_triggered = stop_handler.check_stops(current_prices)
        for sym in stop_triggered:
            price = current_prices[sym]
            shares = holdings.pop(sym)
            stop_handler.remove_position(sym)
            ...
            trade_log.append((date, "SELL_STOP", sym, shares, price, proceeds))

        # 3. Mua / Bán bình thường theo Momentum Rank
        ...
        # Bán (Những mã rớt khỏi TOPK và chưa bị cắt lỗ)
        ...
        if buy_syms:
            ...
            for sym in buy_syms:
                ...
                # Register Stop Loss cho vị thế mới
                stop_handler.add_position(sym, price, date)
...
def main():
    print("=" * 60)
    print("PHASE 5c — BACKTEST VN30 V4 WITH ATR(14) 2.0 STOP-LOSS ONLY")
    print("=" * 60)
    ...
    equity_curve, trade_log = run_backtest_v5c(pred_test, price_map, test_dates)

    metrics, nav_df = compute_tearsheet(equity_curve)

    print("\n" + "=" * 60)
    print("  BACKTEST TEARSHEET V5c (ATR Stop Loss)")
    print("=" * 60)
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "..", "results", "v5c_atr_stop")
